...
function MessyCard({ project, index, boardRef, raise }) {
  ...
  const down = (event) => { const el = card.current; const board = boardRef.current.getBoundingClientRect(); const rect = el.getBoundingClientRect(); drag.current = { active: true, x: event.clientX, y: event.clientY, left: rect.left - board.left, top: rect.top - board.top, moved: false }; el.setPointerCapture(event.pointerId); el.style.left = `${drag.current.left}px`; el.style.top = `${drag.current.top}px`; el.style.zIndex = raise(); };
  const move = (event) => { const d = drag.current; if (!d.active) return; const board = boardRef.current.getBoundingClientRect(); const el = card.current; const left = Math.max(8, Math.min(board.width - el.offsetWidth - 8, d.left + event.clientX - d.x)); const top = Math.max(8, Math.min(board.height - el.offsetHeight - 8, d.top + event.clientY - d.y)); if (Math.abs(event.clientX - d.x) + Math.abs(event.clientY - d.y) > 6) d.moved = true; el.style.left = `${left}px`; el.style.top = `${top}px`; };
...
export default function Projects() {
  ...
  const reset = () => board.current.querySelectorAll(".messy-card").forEach((card) => { card.style.left = card.dataset.x; card.style.top = card.dataset.y; card.style.zIndex = card.dataset.z; });
